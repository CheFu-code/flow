...
export default function FlowConsole({
    ...
    const [activeHeaderPanel, setActiveHeaderPanel] = useState<"settings" | null>(
        null,
    );
    ...
    useEffect(() => {
        ...
        const unreadLabel =
            inbox === 0 ? "" : inbox > 99 ? "99+ · " : `${inbox} · `;

        document.title = `${unreadLabel}Flow Mail`;
    ...
    return (
        <main
            className={`${styles.mailShell} ${themeDensity === "compact" ? styles.compactDensity : ""
                }`}
        >
            <FlowHeader
